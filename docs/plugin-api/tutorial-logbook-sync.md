...
<head>
  ...
  <style>
    body { font-family: var(--tx5dr-font-family); color: var(--tx5dr-foreground); }
...
<body>
  ...
  <script>
    const bridge = window.TX5DR.createBridge();
...
iframe 页面通过 `bridge.invoke(action, data)` 与插件通信。插件端注册对应的处理器：
...
onLoad(ctx) {
  ...
  ctx.ui.registerPageHandler({
    async onMessage(pageId, action, data) {
      ...
      const callsign = ctx.operator.callsign;
...
样式方面，宿主会向 iframe 注入一组 CSS 变量（`--tx5dr-foreground`、`--tx5dr-background` 等），你的页面可以直接使用它们来适配亮暗主题。不提供组件库 -- 用原生 HTML 或你喜欢的任何前端框架都行。
...
- 设置页面用 iframe 实现，通过 `bridge.invoke()` 与插件通信
