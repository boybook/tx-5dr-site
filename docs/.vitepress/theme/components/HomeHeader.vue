...
const languageSwitch = computed(() => {
  if (props.locale === 'zh-CN') {
    return { label: '中文', href: withBase('/en/'), title: '切换到 English' };
  }

  if (props.locale === 'en') {
    return { label: 'EN', href: withBase('/ja/'), title: 'Switch to Japanese' };
  }

  return { label: '日本語', href: withBase('/'), title: '中文に切り替え' };
});
