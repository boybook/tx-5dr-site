...
[LiveKit 与实时语音配置](./livekit) 专门说明 Docker、Linux 服务器版和桌面版统一的实时语音配置方式，包括 `ws-compat`、LiveKit 三种媒体网络模式，以及 FRP / 反向代理场景该如何选。
...
4. 需要配置实时语音、LiveKit、FRP 或反向代理时，继续阅读 [LiveKit 与实时语音配置](./livekit)
